...
\Procedure{Q-Learning}{$\mathcal{S}, \mathcal{A}, \gamma, \alpha, s_0, \text{max\_iter}$}
  ...
  \While{$i < \text{max\_iter}$}
    \State $a \gets \text{select\_action}\bigl(s,\,Q_{\text{old}}\bigr)$
...
- One single network that takes $s$ as input and produces a vector consisting of the $Q$ values for each action (where the $i$th component represents $Q(s,a_i)$); or
...
training in line 15 typically uses squared error as a loss function and
