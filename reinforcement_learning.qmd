...
A different model-free strategy is to search directly for a good policy, rather than first estimating a value function or transition model. This is especially useful when the action space is large or continuous and it is awkward to repeatedly solve $\arg\max_a \mathrm{Q}(s,a)$. The idea is to choose a parameterized family of policies $\pi_\theta(a \mid s)$, differentiable with respect to $\theta$ so that we can improve the policy using gradient-based optimization.

Here, $\pi_\theta(a \mid s)$ is a conditional probability distribution over actions: when the agent is in state $s$, it chooses action $a$ with probability $\pi_\theta(a \mid s)$. Stochastic policies like this naturally incorporate exploration during learning.

Our goal is to choose $\theta$ to maximize the expected cumulative reward $J(\theta)$ obtained by following $\pi_\theta$:
$$
...
Suppose we have collected experience as tuples $(s^{(t)}, a^{(t)}, s^{(t+1)}, r^{(t)})$. Since $\mathrm{T}(s,a,s')$ is a probability, we estimate it by counting:
$$
\hat{\mathrm{T}}(s,a,s') = \frac{\#(s,a,s') + 1}{\#(s,a) + |\mathcal{S}|},
$$
where $\#(s,a,s')$ counts transitions matching $(s,a,s')$ in the data and $\#(s,a)$ counts visits to $(s,a)$.

:::{.column-margin}
The added $1$ and $|\mathcal{S}|$ in numerator and denominator are *Laplace smoothing*: they prevent zero-probability estimates and division by zero, and their effect fades as data accumulates. Conceptually, this is similar to having "initialized" our estimate with uniform random probabilities before making any observations.
:::
