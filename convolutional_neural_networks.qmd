...
All of the filters in the first group are applied to the original image; if there are $k$ such filters, then the result is $k$ new images, which are called *channels*. Now imagine stacking all these new images up so that we have a cube of data, indexed by the channel, as well as by the original row and column indices of the image. The next set of filters in the filter bank will generally be *three-dimensional*: each one will be applied to a sub-range of the row and column indices of the image and to all of the channels.
...
Here is a more complex example of two-dimensional filtering. We have two $3 \times 3$ filters in the first layer, $f_1$ and $f_2$. You can think of each one as "looking" for three pixels in a row, $f_1$ vertically and $f_2$ horizontally. Assuming our input image is $n
  \times n$, then the result of filtering with these two filters is an $2 \times n
  \times n$ tensor (Channel $\times$ Height $\times$ Width). Now we apply a tensor filter (hard to draw!) that "looks for" a combination of two horizontal and two vertical bars (now represented by individual pixels in the two channels), resulting in a single final $1 \times n \times  n$ image.

:::{.margin-note}
When we have a color image as input, we treat it as having three
channels, and hence as an $3 \times n \times n$ tensor.
:::
...
-   *size* of one filter is $m^{l-1} \times k^l \times k^l$ plus $1$ bias value (for this one filter);
...
-   *input tensor size* $m^{l-1} \times n^{l-1} \times n^{l-1}$

-   *padding*: $p^l$ is how many extra pixels -- typically with value 0 -- we add around the edges of the input. For an input of size $m^{l-1} \times n^{l-1} \times n^{l-1}$, our new effective input size with padding becomes $m^{l-1} \times (n^{l-1} + 2 \cdot p^l) \times (n^{l-1} + 2 \cdot p^l)$.

This layer will produce an output tensor of size $m^l \times n^l \times n^l$, where $n^l = \lceil (n^{l-1} + 2 \cdot p^l - (k^l - 1)) / s^l \rceil$. The weights are the values defining the filter: there will be $m^l$ different $m^{l-1} \times k^l \times k^l$ tensors of weight values; plus each filter may have a bias term, which means there is one more weight value per filter. A filter with a bias operates just like the filter examples above, except we add the bias to the output. For instance, if we incorporated a bias term of 0.5 into the filter $F_2$ above, the output would be $(-0.5,0.5,-0.5,0.5, -1.5, 1.5,-0.5,0.5)$ instead of $(-1,0,-1,0,-2,1,-1,0)$.
...
Consider a max pooling layer where both the strides and $k$ are set to be 2. This would map a $3 \times 64 \times 64$ image to a $3 \times 32 \times 32$ image. Note that max pooling layers do not have additional bias or offset values.
