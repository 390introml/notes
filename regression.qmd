...
Before We address both the problem of not being able to invert $({X}^T{X})^{-1}$ and the problem of overfitting using a mechanism called *ridge regression*. We add a regularization term $\|\theta\|^2$ to the OLS objective, with a non-negative scalar value $\lambda$ to control the tradeoff between the training error and the regularization term. Here is the ridge regression objective function: 
...
Suppose that, prior to learning our parameters via ridge regression, we *centered* our feature vectors and our labels.
...
and define our *transformed* training data set as,
...
for all $i=1,\ldots,n$. 
From our centered training data set $\mathcal{D}_\text{centered} = \{ (\tilde{x}^{(i)}, \tilde{y}^{(i)}) \}_{i=1}^n$, we can create our data matrix $\tilde{X} \in \mathbb{R}^{n \times d}$ and vector of labels $\tilde{Y} \in \mathbb{R}^n$.
We can learn our parameters $\theta_\text{ridge}^* \in \mathbb{R}^d$ by computing the gradient of $J_\text{ridge} (\theta)$ with respect to $\theta$ and solving for a closed-form solution. 
Note that we will learn $\theta_\text{ridge}^*$ **without** including the offset $\theta_0$. Later, we'll demonstrate why $\theta_0$ is unnecessary with centered data.

Recall the definition of $J_\text{ridge}(\theta)$:

$$
\begin{aligned}
J_{\text{ridge}}(\theta) &= \frac{1}{n}\sum_{i = 1}^n\left(\theta^T\tilde{x}^{(i)} - \tilde{y}^{(i)}\right)^2 + \lambda\|\theta\|^2 \\ 
& = \frac{1}{n} \left( \tilde{X} \theta - \tilde{Y} \right)^T \left( \tilde{X} \theta - \tilde{Y} \right) + \lambda \theta^T \theta.
\end{aligned}
$$

The gradient $\nabla_\theta J_\text{ridge} (\theta)$ will be equal to the gradient from OLS plus an additional term, $\lambda \nabla_\theta \theta^T \theta = 2 \lambda \theta$:

$$
\begin{aligned}
\nabla_\theta J_\text{ridge} (\theta) &= \frac{2}{n} (\tilde{X}^T \tilde{X} \theta - \tilde{X}^T \tilde{Y}) + 2 \lambda \theta \\
& = \frac{2}{n} \left( (\tilde{X}^T \tilde{X} +  n \lambda {I})\theta - \tilde{X}^T \tilde{Y} \right).
\end{aligned}
$$

We can set the gradient equal to $\boldsymbol{0}$ and solve for $\theta$ to obtain the desired result:

$$
\begin{aligned}
\boldsymbol{0} &= \frac{2}{n} \left( (\tilde{X}^T \tilde{X} +  n \lambda {I})\theta^*_\text{ridge} - \tilde{X}^T \tilde{Y} \right) \\
 (\tilde{X}^T \tilde{X} +  n \lambda {I})\theta^*_\text{ridge} &= \tilde{X}^T \tilde{Y} \\
\theta^*_\text{ridge} &=  (\tilde{X}^T \tilde{X} +  n \lambda {I})^{-1}\tilde{X}^T \tilde{Y}. 
\end{aligned}
$$

This is a very nice result! It looks very similar to the OLS closed-form solution.
The difference is that a "ridge" has been added along the diagonal of the matrix $\tilde{X}^T \tilde{X}$.
So, now we have a hypothesis, $h(x) = \theta_\text{ridge}^{*T} x$. How do we assign a label to a testing feature vector?
We can (1) subtract off the average of our feature vectors in our training data set, $\tilde{x} = x - \bar{x}$, (2) compute $h(\tilde{x})$ to obtain our centered label, and then (3) add $\bar{y}$ so that we can have our uncentered label.

How about that pesky offset? Let's see why it is unnecessary when our data is centered.
We will define $\boldsymbol{1}$ to be an $n$-dimensional column vector where every element is a 1. Then, we can write out the ridge regression objective function as:

$$
    J_\text{ridge}(\theta, \theta_0) = \frac{1}{n} (\tilde{X} \theta + \theta_0 \boldsymbol{1} - \tilde{Y})^T(\tilde{X} \theta + \theta_0 \boldsymbol{1} - \tilde{Y}) + \lambda \theta^T \theta. 
$$

With a little bit of work, one can show that the partial derivative of $J_\text{ridge}(\theta, \theta_0)$ with respect to $\theta_0$ is:

$$
    \frac{\partial J_\text{ridge}(\theta, \theta_0)}{\partial \theta_0} = \frac{2}{n}\left( \boldsymbol{1}^T \tilde{X} \theta - \theta_0 n - \boldsymbol{1}^T \tilde{Y}\right).
$$

Equating the above to 0 and solving for $\theta_0$ gives us:

$$
\begin{aligned}
\theta_{0, \text{ridge}}^* &= \frac{1}{n}\boldsymbol{1}^T \left( \tilde{X} \theta_\text{ridge}^* - \tilde{Y}  \right) \\
\theta_{0, \text{ridge}}^* &= \frac{1}{n} \sum_{i=1}^{n} (\theta_\text{ridge}^{*T} \tilde{x}^{(i)} - \tilde{y}^{(i)}).
\end{aligned}
$$ 

However, by construction, $\frac{1}{n} \sum_{i=1}^n \tilde{x}^{(i)} = \boldsymbol{0}$ and $\frac{1}{n} \sum_{i=1}^n \tilde{y}^{(i)} = 0$, meaning that $\theta_{0, \text{ridge}}^* = 0$. Hurray!
