...
These properties are often in tension --- for instance, maximizing compactness may sacrifice explanatory power. Much of representation learning is about navigating these trade-offs.
...
**Hierarchical feature learning.** Work by Zeiler and Fergus (2014) visualized what individual neurons in a trained CNN respond to. The findings were striking:

- **Layer 1** neurons respond to simple edges and colors --- much like Gabor filters long used in classical computer vision.
- **Layer 2** neurons combine these into textures and simple patterns (corners, circles, stripes).
- **Layers 3--4** respond to object parts (wheels, eyes, text) and increasingly complex combinations.
- **Layer 5** neurons respond to entire objects and scenes (specific dog breeds, flowers, faces).

:::{.column-margin}
This progression mirrors the classical computer vision pipeline --- edges, textures, segments, parts, objects --- except that here the features are *learned* end-to-end from data rather than hand-designed.
:::

This hierarchy also echoes what neuroscientists observe in the brain's visual system, which processes information through a series of areas with increasing complexity --- from simple edge detectors in early visual cortex to neurons that respond to whole objects in higher areas. Remarkably, when researchers compare how a deep network and the primate brain each organize visual information, the similarity structures turn out to be strikingly alike: images that produce similar neural activation patterns in the brain also produce similar activation patterns in the deep network, and vice versa.

:::{.column-margin}
Yamins et al. (PNAS 2014) compared the similarity structure of deep network activations with neural recordings from primate visual cortex and found a strong correspondence --- deep nets organize visual information much like our brains do.
:::

**Representations as embeddings.** Each layer of a neural network maps its input to a vector of activations. We can think of this vector as an *embedding* --- a point in a high-dimensional space where position encodes meaning. For example, given an image $x$, the activations at some intermediate layer form a vector that represents the image at a particular level of abstraction. We might call this "im2vec" by analogy with "word2vec" (discussed in @sec-embeddings below). If the network has learned a good representation, then semantically similar images will have embeddings that are close together --- two photos of golden retrievers will be nearby in this space, even if the actual pixel values are very different.
...
### Autoencoders {#sec-autoencoder-structure}

*Autoencoders* are a classical approach to learning representations from unlabeled data. The idea is to learn a compressed version of the data by training a network to reconstruct its own input through a bottleneck.
...
Autoencoders learn representations by compressing and reconstructing data. *Self-supervised learning* takes a broader approach: instead of reconstruction, it creates a supervised learning problem *from the data itself*, using the structure of the data to generate labels automatically.

The key idea is to design a *pretext task* --- an auxiliary prediction problem whose labels are free. By training a network to solve the pretext task, useful representations emerge in its internal layers.
...
### Contrastive learning (optional) {#sec-contrastive}

:::{.callout-note}
This section covers material beyond the scope of exams. It is included for interested readers who want a broader picture of modern representation learning methods.
:::

*Contrastive learning* takes a different approach to learning without labels: rather than reconstructing or predicting parts of the input, it learns by *comparing* examples.
...
### Multimodal learning (optional) {#sec-multimodal}

:::{.callout-note}
This section covers material beyond the scope of exams. It is included for interested readers who want a broader picture of modern representation learning methods.
:::

The methods above learn representations from a single type of data (images, or text). *Multimodal learning* extends this idea by learning shared representations across different data types --- for instance, images and their text descriptions, or audio and video.

The intuition is that different modalities provide complementary views of the same underlying concept. A photo of a dog and the sentence "a golden retriever playing fetch" both refer to the same scene. By training a model to align representations across modalities, we obtain a shared embedding space where related concepts from different modalities are close together, regardless of their original form.
...
## Emergence of meaning from structure {#sec-emergence}

A striking theme across all these methods --- autoencoders, self-supervised learning, contrastive learning --- is that *semantic meaning emerges from structural regularities in the data, not from labels*.

An autoencoder trained on digit images, with no knowledge of digit labels, learns a latent space where images of the same digit cluster together. A language model trained to predict the next word, with no explicit grammar instruction, learns syntactic and semantic structure. A contrastive model trained with only image augmentations learns to distinguish object categories it was never told about.

Why does this happen? Because *the factors that explain the structure of the data are often the same factors that are semantically meaningful*. Digit identity explains most of the variation among digit images. Semantic relationships explain most of the predictability in text. The learning algorithms discover these factors not because they are told to, but because doing so is the most efficient way to solve their training objectives --- compression, prediction, or comparison.

This also helps explain why some pretext tasks work better than others. Tasks that require understanding *meaning* to solve (predicting masked words, reconstructing missing image regions) tend to produce better representations than tasks that can be solved by surface-level shortcuts (e.g., simply memorizing pixel statistics).


## Embeddings {#sec-embeddings}

The representations learned by the methods above are, at their core, *embeddings*: mappings from complex, high-dimensional data (images, words, sentences, graphs) into vectors in a lower-dimensional space. The key property of a good embedding is that **geometric relationships in the vector space reflect meaningful relationships in the original data**.

### Word embeddings

In natural language processing, words are represented as vectors, commonly known as *word embeddings*. Semantically related words such as "dog" and "cat" should have vectors close together, while unrelated words like "cat" and "table" should be farther apart.

Similarity between embeddings is frequently measured using the *inner product* (dot product):
...
The same principles apply far beyond text:
