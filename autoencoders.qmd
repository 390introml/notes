...
The methods above --- self-supervised pretext tasks and autoencoders --- learn representations by predicting or reconstructing data. *Contrastive learning* takes a different approach: rather than reconstructing or predicting parts of the input, it learns by *comparing* examples, directly shaping the geometry of the representation space.
...
Despite the diversity of methods we have encountered, they all share a common goal: imposing constraints that reveal latent structure in data. The differences lie in *what constraint* is used:
...
| **Context** | Co-occurring elements should map nearby | Word2Vec, Node2Vec (below) |
...
### Learning representations through context {#sec-context-embeddings}

One paradigm not yet covered in detail is *context-based* representation learning, where embeddings are shaped by co-occurrence: elements that appear in similar contexts should have similar representations.

In natural language processing, this idea produces *word embeddings*. Semantically related words such as "dog" and "cat" should have vectors close together, while unrelated words like "cat" and "table" should be farther apart.
