...
Next, suppose our given dictionary has $n$ number of entries/entries, we embed each one of these into a so-called key vector. In particular, for the $j$th entry in the dictionary, we produce a $k_j \in \mathbb{R}^{d_k\times 1}$ key vector, where $j=1,2,3, \dots, n.$

We can then apply a softmax (@sec-classification, @sec-neural_networks) to the inner-product between the key and query:

$$\text{softmax}\left(
  \alpha = [q^T k_1; q^T k_2; q^T k_3; \dots, q^T k_n]\in\mathbb{R}^{d_n}
  \right)$$
...
The meaning of this weighted average value may be ambiguous when the values are just words. However, the attention output really becomes meaningful when the values are projected in some semantic embedding space as the query and keys (and such projections are typically done in transformers via learned embedding weights).
