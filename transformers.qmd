...
The image above illustrates auto-regressive training, where the model learns to predict the next token in a sequence based on the preceding tokens. Each token's prediction is conditioned on the context provided by the previous tokens, allowing the model's predicted probability distribution over the next token to be computed using the prior ones as input — not just the most recent one. This can be written formally as: 
$$
...
- **Key vector** $k_j$: Represents the "answer" that other tokens provide to queries.
- **Value vector** $v_j$: Represents the actual information that is aggregated based on attention scores.
...
### Training: Parallel Processing Over the Whole Sequence
...
### Inference: Sequential Token-by-Token Generation
...
### Summary: Training vs. Inference at a Glance
