...
2. **A feed-forward network (FFN)**, which applies a nonlinear transformation independently to each token's representation.
...
The FFN is applied position-wise — meaning the same two-layer MLP is applied to every token independently:

$$
\text{FFN}(x_i) = \text{ReLU}(x_i W_1 + b_1) W_2 + b_2
$$

where $W_1 \in \mathbb{R}^{d \times d_{ff}}$ and $W_2 \in \mathbb{R}^{d_{ff} \times d}$. The hidden dimension $d_{ff}$ is typically set to $4d$ — so for a model with $d = 512$, the FFN expands each token to 2048 dimensions before projecting back. This expansion is where much of the model's expressive capacity lives.

:::{.column-margin}
A useful way to think about the division of labor: attention layers *mix* information across positions (each token can gather context from anywhere in the sequence), while the FFN layers *deepen* each position's representation independently (each token is individually transformed by a powerful nonlinear function). Both are necessary: attention without FFNs lacks expressive depth; FFNs without attention cannot share information across positions.
:::

### Residual Connections and Layer Normalization

**Residual connections** (the $X + \text{Sublayer}(X)$ pattern) allow gradients to flow directly through many layers without vanishing, making it practical to stack dozens or even hundreds of transformer blocks. In effect, each block only needs to learn the *correction* to the current representation rather than a full transformation from scratch.

**Layer normalization** stabilizes training by normalizing the activations within each token's representation to have zero mean and unit variance at each layer. This prevents the scale of activations from exploding or collapsing across layers, which is especially important in very deep models.
...
3. **Add positional embeddings** $p_i$ to each token embedding: $x_i^* = x_i + p_i$.
4. **Pass through $L$ transformer blocks**, each consisting of causal multi-head attention + FFN + residual connections + layer normalization.
5. **Project** the final token representations to logits over the vocabulary using $W_E^T$ (often the transposed embedding matrix is reused here) and apply softmax to get a probability distribution over the next token.
