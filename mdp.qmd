...
MDPs satisfy the Markov property: the next-state distribution depends only on the current state and action, not on the past. Formally, for all $t \geq 0$,

$$\Pr(S_{t + 1}|S_t, A_t, S_{t - 1}, A_{t - 1}, \ldots, S_0, A_0) = \Pr(S_{t + 1}|S_t, A_t)\;\;.
$$

That is, the future depends only on the current state and action, not the full history.
