...
$$
\frac{dL} = \frac{dL}{dg}^T \cdot (dW)^Tx
$$
...
\begin{aligned}
\frac{dL} &= \frac{dL}{dg}^T \cdot (dW)^Tx
          &= tr( \frac{dL}{dg}^T \cdot (dW)^Tx)
          &= tr( x^T \cdot dW \cdot \frac{dL}{dg})
          &= tr( \frac{dL}{dg}  \cdot x^T \cdot dW)
          &= tr( (x \cdot \frac{dL}{dg}^T)^T \cdot dW)
\end{aligned}
$$

where the second line uses the fact that the RHS is a scalar, the third uses the fact that the trace is invariant to transposing its input, the third uses the cyclic property, and the fourth simply combines the terms to match the form of @eq-dL-trace.
...
where x is the input into that weight matrix and $g$ is the output from it. 