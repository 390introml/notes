...
What our inputs and outputs are not just vectors, but are matrices? In this case, things certainly become more complicated. For instance, note that the number of elements needed in a derivative is the product of the number of elements in the input and output of the function. That's why vector-by-vector derivatives look like matrices. This would mean that vector-by-matrix derivatives would require us to have a three-dimensional derivative object (often referred to as a tensor). These types of high-order objects are beyond the scope of this course. 

However, we will deal w/ one specific case of matrix derivatives; namely, when we have a function $f$ that maps inputs of size $m \times n$ to scalar outputs. This derivative will require only a matrix of values to capture its information. But how do we formally define this? 

We can extend the notation above of accumulating partial derivatives quite naturally to this case. Specifically:

$$df = \sum_{i,j} \frac{df}{dA_{i,j}} \cdot dA_{i,j}$$

In other words, we element-wise multiply the elements of $\frac{df}{dA}$ and $dA$ and sum up the results! This is kind of look a dot-product for matrices, and immediatly tells us that the size of $\frac{df}{dA}$ must match the size of $dA$. 
...
::: {.callout-note collapse="true" title="Numerator Layout"}
Consider the matrix $X$ of size $m \cross n$, and denote $X_j$$ as the jth column of X. We have:
$$
\begin{aligned}

  df &= tr(\frac{df}{dA}^T \cdot dA)
     &= \sum_{j=1}^{n} \frac{df}{dA}_j \cdot dA_j
