...
Let us use the same exact same format as before: 
...
We are **intentionally** adding a transpose to the $\frac{df}{dx}$ term. This means that $\frac{df}{dx}$ in this case will be of size $(n \times m)$, which is the transpose of how things were defined in the Numerator Layout. This means that the input ($x$) determines the rows in the derivative, and the output ($f$) determine the columns in the derivative. In other words:
...
Calculating this derivative is basically the same as last time, expect there's an additional squared term at the end that we can differentiate separately. Setting the derivative with respect to $\theta$ to 0, gives:
