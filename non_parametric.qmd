...
\begin{algorithm}
  \begin{algorithmic}[1]
    \Procedure{RandomForest}{$\mathcal{D}, B, m, n$}
      \For{$b = 1, \ldots, B$}
        \State Draw a bootstrap sample $\mathcal{D}_b$ of size $n$ from $\mathcal{D}$
        \State Grow tree $T_b$ on data $\mathcal{D}_b$ recursively:
        \begin{itemize}
          \item Select $m$ variables at random from the $d$ available variables
          \item Pick the best variable and split point among the $m$ variables
          \item Split the node into child nodes
        \end{itemize}
      \EndFor
      \State \Return ensemble of trees $\{T_b\}_{b=1}^{B}$
    \EndProcedure
  \end{algorithmic}
\end{algorithm}
```
